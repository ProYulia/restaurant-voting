...
import com.github.proyulia.error.*;
...
import static com.github.proyulia.error.ErrorType.*;
...
public class RestExceptionHandler {
    ...
    protected ProblemDetail processException(@NonNull Exception ex,
        ...
        if (optType.isPresent()) {
            ...
            return createProblemDetail(ex, optType.get(), ex.getMessage(),
                    additionalParams);
        } else {
            Throwable root = getRootCause(ex);
            log.error(ERR_PFX + "Exception " + root + " at request " + path,
                    root);
    ...
    private ProblemDetail createProblemDetail(Exception ex,
        ...
        ErrorResponse.Builder builder = ErrorResponse.builder(ex, type.status
                , defaultDetail);
        ProblemDetail pd = builder.build()
                .updateAndGetBody(messageSource,
